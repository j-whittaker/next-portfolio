...
const SongDataCol : React.FC<{ content : string, hover: ((ref : RefObject<HTMLSpanElement>, hover: boolean) => void) | null, additionalClasses?: string}>  = ({ content, hover, additionalClasses = ''}) => {
...
const Track : React.FC<{name: string, artists: string[], album: string, url: string, additionalClasses?: string, disableHover?: boolean }> = ({name, artists, album, url, additionalClasses = '', disableHover = false}) => {
    ...
    return <div onClick={() => {openUrlNewWindow(url);}} className={`relative track !m-0 text-xs text-white max-w-full w-full flex max-h-8 gap-3 !px-2 !py-0 hover:cursor-pointer hover:bg-secondary border-b-2 border-b-secondary last:border-b-0 ${additionalClasses}`}>
