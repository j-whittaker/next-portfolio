...
function SlideBtn({setActiveSlide, active }: { setActiveSlide: any; active: string; }) {
...
function TestimonialSlide({quote, author, active }: { quote: any;  author: any;  active: any;}) {
  return (<>
...
const TestimonialSlider : React.FC = () => {
  ...
  const [intervalId, setIntervalId] = useProperty(-1);
  ...
  const setActiveSlideClick = useCallback((idx : number, intervalId : number ) => {
    if(intervalId) clearInterval(intervalId);
